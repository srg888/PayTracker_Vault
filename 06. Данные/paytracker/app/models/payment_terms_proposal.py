...
class PaymentTermsProposal(Base):
    """История предложений условий исполнения платежа (способ, комиссия, курс) —
    см. "00a. Согласование условий исполнения (Платёж).md", Бизнес-правила BR-100..102.

    Отдельная от PaymentRequest таблица (1:N), а не поля в самой заявке — чтобы не
    терять историю при повторном предложении после отклонения Заказчиком (BR-101)."""
    ...
    decision_comment: Mapped[str | None] = mapped_column(Text, nullable=True)  # обязателен при отклонении (BR-101)
