...
var GitHubOwnerSuggest = class extends import_obsidian2.AbstractInputSuggest {
...
var GitHubRepoSuggest = class extends import_obsidian2.AbstractInputSuggest {
  constructor(app, inputEl) {
    super(app, inputEl);
    this.repoList = [];
    this.inputEl = inputEl;
...
var GitignoreFilter = class _GitignoreFilter {
  constructor(filters) {
  ...
  ignores(filePath) {
    ...
    for (const dirPath of dirsToCheck) {
      ...
      if (!filter) continue;
      const relativePath = dirPath === "" ? filePath : filePath.substring(dirPath.length + 1);
...
function deriveGitignorePaths(filePaths) {
  const gitignorePaths = /* @__PURE__ */ new Set();
...
async function loadFilters(adapter, gitignorePaths, knownPaths) {
  const filters = /* @__PURE__ */ new Map();
  await Promise.all(
    Array.from(gitignorePaths).map(async (gitignorePath) => {
      try {
        const knownToExist = knownPaths == null ? void 0 : knownPaths.has(gitignorePath);
        if (!knownToExist) {
          const stat = await adapter.stat(gitignorePath);
          if (!stat || stat.type !== "file") return;
        }
        const content = await adapter.read(gitignorePath);
        const dirPath = gitignorePath === ".gitignore" ? "" : gitignorePath.replace(/\/.gitignore$/, "");
        filters.set(dirPath, (0, import_ignore.default)().add(content));
      } catch (e) {
      }
    })
  );
...
async function getCachedDerivedKey(usage) {
  if (cache[usage] === null) {
...
async function encryptContent(content) {
  ...
  return (0, import_obsidian4.arrayBufferToBase64)(combined.buffer);
}
async function decryptContent(encryptedContent) {
  const key = await getCachedDerivedKey("decrypt");
  const combined = new Uint8Array((0, import_obsidian4.base64ToArrayBuffer)(encryptedContent));
...
async function encryptPath(path) {
  const key = await getCachedDerivedKey("encrypt");
  const segments = path.split("/");
  const encryptedSegments = await Promise.all(
    segments.map(async (seg) => {
      ...
      return (0, import_obsidian4.arrayBufferToBase64)(encrypted).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
    })
  );
  return encryptedSegments.join("/");
}
async function decryptPath(encryptedPath) {
  const key = await getCachedDerivedKey("decrypt");
  ...
  const decryptedSegments = await Promise.all(
    segments.map(async (seg) => {
      ...
      const data = (0, import_obsidian4.base64ToArrayBuffer)(paddedBase64);
      const fixedIv = new Uint8Array(12).fill(0);
      const decrypted = await crypto.subtle.decrypt(
        { name: "AES-GCM", iv: fixedIv },
        key,
...
var RemoteGitHubVault = class {
  ...
  async readFileContent(path) {
    ...
    try {
      ...
      if (typeof blob.content !== "string") {
        throw new Error(
          `Cannot read '${path}': blob content is ${typeof blob.content} (encoding: '${(_a = blob.encoding) != null ? _a : "unknown"}'). GitHub may not support this blob format.`
        );
      }
  ...
  async createTree(treeNodes, base_tree_sha) {
    ...
    try {
      ...
      let updatedTree = treeNodes;
      if (isEnabled()) {
        updatedTree = await Promise.all(
          treeNodes.map(async (node) => ({
            ...node,
            path: await encryptPath(node.path)
          }))
        );
      }
...
` + suspiciousMatches.map(
   ...
   * If the latest commit SHA matches the cached SHA, returns cached state immediately.
   */
  async readFromSource(ignoreCache = false) {
    const { commitSha, treeSha } = await this.getLatestCommitAndTreeSha();
...
var FitNotice = class {
  ...
  show(initialMessage, addClasses = [], duration = 0) {
    if (!this.notice && !this.muted) {
      ...
      this.notice = new import_obsidian5.Notice(message, duration);
  ...
  setMessage(message, isError) {
    if (isError) {
      if (!this.notice) {
        this.notice = new import_obsidian5.Notice(message, 0);
...
function showFileChanges(records) {
  ...
  const fileOpsNotice = new import_obsidian6.Notice("", 0);
...
function showUnappliedConflicts(clashedFiles) {
  ...
  const conflictNotice = new import_obsidian6.Notice("", 0);
...
// src/fitSetting.ts
init_logger();
var FitSettingTab = class extends import_obsidian7.PluginSettingTab {
  constructor(app, plugin2) {
    ...
    this.suggestedRepos = [];
    this.authenticateButtonComponent = null;
    this.refreshButton = null;
    this.ownerInputComponent = null;
    this.repoInputComponent = null;
    this.ownerSuggest = null;
    this.repoSuggest = null;
    /**
     ...
     */
    this.debouncedFetchReposForOwner = (owner) => {
    ...
    this.githubUserInfoBlock = () => {
      ...
      new import_obsidian7.Setting(containerEl).setHeading().setName("GitHub user info").addButton((button) => {
        this.authenticateButtonComponent = button;
        button.setCta().setButtonText("Authenticate user").setDisabled(this.authenticating || !this.plugin.githubConnection).onClick(async () => {
          if (this.authenticating) return;
          await this.handleUserFetch();
        });
      });
      this.authUserSetting = new import_obsidian7.Setting(containerEl).setDesc("Input your personal access token below to get authenticated. Create a GitHub account here if you don't have one yet.").addExtraButton((button) => button.setIcon("github").setTooltip("Sign up on github.com").onClick(async () => {
      ...
      this.patSetting = new import_obsidian7.Setting(containerEl).setName("Github personal access token").setDesc('Make sure Permissions has Contents: "Read and write". Recommended: Limit to selected repository, adjust expiration.').addText((text) => text.setPlaceholder("GitHub personal access token").setValue(this.plugin.settings.pat).onChange(async (value) => {
    ...
    this.repoInfoBlock = async () => {
      const { containerEl } = this;
      const isAuthenticated = !!this.plugin.githubConnection;
      new import_obsidian7.Setting(containerEl).setHeading().setName("Repository info").setDesc(isAuthenticated ? "Suggestions populate automatically when authenticated. Click refresh to update the lists." : "Authenticate above to populate owner/repo suggestions, or type custom values manually.").addExtraButton((button) => {
        this.refreshButton = button.extraSettingsEl;
        button.setTooltip("Refresh repos and branches list").setDisabled(!isAuthenticated).setIcon("refresh-cw").onClick(async () => {
          await this.refreshFields("repo(0)");
        });
      });
      new import_obsidian7.Setting(containerEl).setDesc("Make sure you are logged in to github on your browser.").addExtraButton((button) => button.setIcon("github").setTooltip("Create a new repository").onClick(() => {
        window.open(`https://github.com/new`, "_blank");
      }));
      this.ownerSetting = new import_obsidian7.Setting(containerEl).setName("Repository owner").setDesc(isAuthenticated ? "The GitHub username or organization that owns the repository. Select from suggestions or type a custom value." : "Type a custom value, or authenticate above to see suggestions.");
      this.ownerSetting.addText((text) => {
        this.ownerInputComponent = text;
        text.setPlaceholder(isAuthenticated ? "owner-username" : "Authenticate above to auto-fill").setValue(this.plugin.settings.owner).onChange(async (value) => {
          var _a;
          ...
          if (ownerChanged) {
            this.plugin.settings.repo = "";
            this.plugin.settings.branch = "";
            (_a = this.repoInputComponent) == null ? void 0 : _a.setValue("");
            this.existingRepos = [];
            if (this.repoSuggest) {
              this.repoSuggest.updateSuggestions([]);
            }
      ...
      this.repoSetting = new import_obsidian7.Setting(containerEl).setName("Repository name").setDesc(isAuthenticated ? "Select a repo to sync your vault or type a custom value. Refresh to see your latest repos." : "Authenticate above to see repo suggestions, or type a custom value.");
      this.repoSetting.addText((text) => {
        this.repoInputComponent = text;
        text.setPlaceholder(isAuthenticated ? "repo-name" : "Authenticate above for suggestions").setValue(this.plugin.settings.repo).onChange((value) => {
      ...
      new import_obsidian7.Setting(containerEl).setName("Branch name").setDesc("Enter repository details above, then refresh to view existing branches.").addDropdown((dropdown) => {
      ...
      const linkDisplay = new import_obsidian7.Setting(containerEl).setName("View your vault on GitHub").setDesc(this.repoLink).addExtraButton(
      ...
      function message(notice, string, error = null) {
      ...
      const setting = new import_obsidian7.Setting(containerEl).setName("Encryption password (Advanced only)").setDesc(
      ...
      setting.addExtraButton((button) => button.setIcon("eye").setTooltip("Switch visibility").onClick(() => {
        visible = !visible;
        textComponent.inputEl.type = visible ? "text" : "password";
        button.setIcon(visible ? "eye-off" : "eye");
      }));
      setting.addExtraButton((button) => button.setIcon("arrow-right").setTooltip("Apply password").onClick(async () => {
      ...
      new import_obsidian7.Setting(containerEl).setName("Sync local cache").setDesc("Re-read remote state into local cache. (Advanced use only \u2014 encryption migration or cache recovery)").addExtraButton((button) => button.setIcon("refresh-cw").setTooltip("Sync local cache").onClick(async () => {
        ...
        try {
          ...
          notice.remove("done", 5e3);
        } catch (e) {
          message(notice, "Failed to update cache", e);
          notice.remove("error", 5e3);
        } finally {
          button.setDisabled(false);
        }
      }));
      new import_obsidian7.Setting(containerEl).setName("Clear repository").setDesc("Delete all files from the repository. Previous commits unaffected. (Advanced use only \u2014 encryption migration)").addExtraButton((button) => button.setIcon("trash-2").setTooltip("Clear repository").onClick(async () => {
        button.setDisabled(true);
        const notice = new FitNotice(
          this.plugin.fit,
          ["loading"],
    ...
    this.localConfigBlock = () => {
      const { containerEl } = this;
      new import_obsidian7.Setting(containerEl).setHeading().setName("Local configurations");
      new import_obsidian7.Setting(containerEl).setName("Device name").setDesc("Sign commit message with this device name.").addText((text) => text.setPlaceholder("Device name").setValue(this.plugin.settings.deviceName).onChange(async (value) => {
        this.plugin.settings.deviceName = value;
        await this.plugin.saveSettings();
      }));
      new import_obsidian7.Setting(containerEl).setName("Auto sync").setDesc(`Automatically sync your vault when remote has updates. (Muted: sync in the background without displaying notices, except for file changes and conflicts notice)`).addDropdown((dropdown) => {
      ...
      const checkIntervalSlider = new import_obsidian7.Setting(containerEl).setName("Auto check interval").setDesc(`Automatically check for remote changes in the background every ${this.plugin.settings.checkEveryXMinutes} minutes.`).addSlider(
    ...
    this.noticeConfigBlock = () => {
      ...
      const makeItem = (name, getValue, setValue, renderPreview) => {
      ...
      makeItem(
        ...
        async (v) => {
          this.plugin.settings.notifyChanges = v;
          await this.plugin.saveSettings();
        },
        (el) => {
      ...
      makeItem(
        ...
        async (v) => {
          this.plugin.settings.notifyConflicts = v;
          await this.plugin.saveSettings();
        },
        (el) => {
      ...
      new import_obsidian7.Setting(containerEl).setName("Enable debug logging").setDesc(`Write detailed sync logs to ${this.plugin.manifest.dir}/debug.log. Useful for troubleshooting and bug reports.`).addToggle((toggle) => toggle.setValue(this.plugin.settings.enableDebugLogging).onChange(async (value) => {
...
Additionally, logging to debug.log failed. Check the console for details.`;
      new import_obsidian8.Notice(noticeMessage, 0);
    } catch (err) {
      noticeError = err;
        ...
        try {
          ...
          if (errorObj.status === 404) {
            ...
            while (hasMorePages) {
              ...
              const matchingRepos = response.filter((r) => r.owner.login === owner).map((r) => r.name);
...
var FitPlugin = class extends import_obsidian9.Plugin {
  constructor() {
    super(...arguments);
